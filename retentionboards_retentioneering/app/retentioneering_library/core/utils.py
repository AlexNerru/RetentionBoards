...
from retentioneering_library.core import feature_extraction
from retentioneering_library.core import clustering
from retentioneering_library.visualization import plot, funnel
from sklearn.linear_model import LogisticRegression
from retentioneering_library.core.model import ModelDescriptor
from retentioneering_library.core import node_metrics
from retentioneering_library.core import preprocessing
...
def init_config(**config):
    ...
    -------
    index_col: str
        Dataframe index column name (e.g. `user_pseudo_id` in our examples). Index column is the ID of trajectory which is a series of events or states happening at given times. Index column separates your dataset by users or sessions, etc. The most common ones are user or session columns. If you have a default dataset with user_id, event_name and timestamp columns, and want to use session as an index column, please use ``retentioneering_library.core.preprocessing.split_sessions`` method to generate pseudosessions.
    event_col: str
...
class BaseDataset(BaseTrajectory):
    ...
    def extract_features(self, feature_type='tfidf', drop_targets=True, metadata=None, **kwargs):
        ...
        feature_type: str, optional
            ...
            - TFIDF (``feature_type='tfidf'``). For more information refer to ``retentioneering_library.core.feature_extraction.tfidf_embedder``.
            - Event frequencies (``feature_type='frequency'``). For more information refer to ``retentioneering_library.core.feature_extraction.frequency_embedder``.
    ...
    def show_tree_selector(self, **kwargs):
        ...
        """
        self._init_cols(locals())
        from retentioneering_library.core.tree_selector import show_tree_filter
    ...
    def use_tree_filter(self, path, **kwargs):
        ...
        pd.DataFrame
        """
        from retentioneering_library.core.tree_selector import use_tree_filter
